...
export async function resolveTimeOrLabel(bridge: Bridge, input: string): Promise<number> {
  ...
  throw new Error(`"${input}" is ambiguous: ${matches.map((t) => t.label).join(', ')} — be more specific.`);
}
...
export function registerPlaybackTools(server: McpServer, bridge: Bridge): void {
  server.registerTool(
    'play',
    {
      ...
      inputSchema: { from: z.string().optional().describe('Time or saved timestamp label — omit to resume in place') },
    },
    handler(async ({ from }) => {
      if (from !== undefined) {
  ...
  server.registerTool(
    'seek',
    {
      description: 'Jump to a position without changing play/pause state. Accepts a time or a saved timestamp label.',
      inputSchema: { to: z.string().describe('Time ("1:30", "90", "1m30s") or saved timestamp label') },
    },
    handler(async ({ to }) => {
      const seconds = await resolveTimeOrLabel(bridge, to);
      const state = await bridge.send('seek', { seconds });
      return ok(`Now at ${formatTime(state.currentTime)} (${state.paused ? 'paused' : 'playing'}).`);
    }),
  );
  ...
  server.registerTool(
    'set_speed',
    {
      description: 'Set playback speed. Accepts 0.25–2.0, with or without an "x" suffix (e.g. "0.75x").',
  ...
  server.registerTool(
    'set_volume',
    {
      description: 'Set the video volume (the player element only, not system volume). Absolute 0–100, or relative "+10"/"-10".',
      inputSchema: { level: z.union([z.string(), z.number()]).describe('0–100, or "+10"/"-10" for relative change') },
    },
    handler(async ({ level }) => {
